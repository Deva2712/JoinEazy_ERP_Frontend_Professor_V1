// src/pages/StudentResearch/views/ExploreResearchStudentView.jsx

import React from "react";
import { Search, X, SlidersHorizontal } from "lucide-react";
import ResearchCard from "../components/ResearchCard";

const ExploreResearchStudentView = ({
	availableProjects = [],
	...
	// Combine projects and publications
	const allItems = [
		...availableProjects.map((p) => ({ ...p, type: "project" })),
		...availablePublications.map((p) => ({ ...p, type: "publication" })),
	];

	// Apply type filtering
	const filteredByType = allItems.filter((item) => {
		if (item.type === "project" && !filters.showProjects) return false;
		if (item.type === "publication" && !filters.showPublications)
			return false;
		return true;
	});
			...
			{filteredByType.length === 0 ? (
				<div className="flex flex-col items-center justify-center py-20 text-center">
					...
					<p className="text-gray-600 dark:text-gray-400">
						Try adjusting your search or filters to find research opportunities.
					</p>
				</div>
			) : (
...
export default ExploreResearchStudentView;