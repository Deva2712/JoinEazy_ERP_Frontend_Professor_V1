...
//   History, User, UserPlus, AlertTriangle,
// } from "lucide-react";
...
//       return substitutionRequests.filter((s) => s.status !== "Rejected" && new Date(s.toDate) >= today);
//     }
...
//     { label: "Substitution Requests", value: String(substitutionRequests.filter((s) => s.status === "Pending").length), icon: UserPlus },
//   ];
...
const SUBSTITUTION_SUB_TABS = [
  { key: "my-substitutes", label: "My Substitutes", icon: Users   },
  { key: "substitutes",    label: "Substitutes",     icon: UserPlus },
];
...
const LeaveApplicationUI = ({
  ...
  const outgoingSubstitutions = applications.filter((a) => !!a.replacementFaculty);
